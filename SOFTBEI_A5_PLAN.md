...
产品表达上叫“路线规划智能体”，工程实现上保持受约束的规则评分与可解释决策链，不让 LLM 自由编造路径。根据兴趣、游玩时长、同行类型、体力偏好、当前位置、必去景点和拥挤度偏好生成路线。路线不是普通列表，而是“点位顺序 + 每站讲解重点 + 表演时间提醒 + 预计停留 + 必去/可替代约束 + 推荐理由 + 置信度/评分拆解”。
...
3. 路线推荐置信度与决策链：把推荐从模板列表升级成可解释智能决策。
4. 自然语言路线推荐：游客直接说“我带老人孩子，3 小时，别太挤”，系统解析偏好后调用受约束路线规划器。
5. Route Memory Agent：记住游客偏好、必去景点、上一条路线、删减原因和最近对话，支持“太累了/缩短一点/从这里出发/灵山大佛一定要去”等多轮重规划。
6. 智能讲解深浅调节：同一份 RAG 来源可输出亲子版、历史深度版、30 秒短讲版、拍照打卡版。
7. 低置信度澄清追问：检索不足、意图不清或多景点歧义时先追问，不直接编造。
8. 评测看板：展示 QA、识景、路线推荐、自然语言意图解析和低置信追问的测试集通过率，强化可信度。
9. 知识缺口闭环：后台发现低置信问题，一键生成待补充知识条目。
10. 弱网离线兜底：无 API key 或弱网时仍可使用预置 FAQ、路线模板和本地讲解。
...
- Route Planner Agent：受约束路线规划器。内部由主题识别、必去/可选/避开景点约束、候选景点过滤、时间/体力/人群/拥挤度评分、站点排序裁剪、置信度拆解组成；LLM 只用于自然语言解释和讲解润色，不直接决定路径。
