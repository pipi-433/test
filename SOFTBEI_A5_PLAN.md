...
4. 游客自然语言能力矩阵：把景区总览、兴趣推荐、景点/景区对比、拥挤运营状态、RAG 问答、路线规划和资料外兜底统一放进 Query Understanding Gate 分流。
5. 路线推荐置信度与决策链：把推荐从模板列表升级成可解释智能决策。
6. 自然语言路线推荐：游客直接说“我带老人孩子，3 小时，别太挤”，系统解析偏好后调用受约束路线规划器。
7. Route Memory Agent：记住游客偏好、必去景点、上一条路线、删减原因和最近对话，支持“太累了/缩短一点/从这里出发/灵山大佛一定要去”等多轮重规划。
8. 智能讲解深浅调节：同一份 RAG 来源可输出亲子版、历史深度版、30 秒短讲版、拍照打卡版。
9. 低置信度澄清追问：检索不足、意图不清或多景点歧义时先追问，不直接编造。
10. 评测看板：展示 QA、识景、路线推荐、自然语言意图解析和低置信追问的测试集通过率，强化可信度。
11. 知识缺口闭环：后台发现低置信问题，一键生成待补充知识条目。
12. 运营事件控制台：管理员手动发布拥挤、临时关闭、演出提醒或分流建议，路线规划器即时响应。
13. 导流效果模拟看板：展示推荐前后热门点压力变化和替代景点曝光提升，明确标注为模拟导流效果。
14. 游客路线小票/游记卡：Kiosk 扫码带走路线时生成可读的小票式路线摘要，游后可生成轻量回顾卡。
15. AI 内容与隐私提示：游客端和 Kiosk 明确提示 AI 生成、来源优先、公共终端不保存个人身份信息。
16. 弱网离线兜底：无 API key 或弱网时仍可使用预置 FAQ、路线模板和本地讲解。
