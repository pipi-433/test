...
产品表达上叫“路线规划智能体”，工程实现上保持受约束的规则评分与可解释决策链，不让 LLM 自由编造路径。根据兴趣、游玩时长、同行类型、体力偏好、当前位置和拥挤度偏好生成路线。路线不是普通列表，而是“点位顺序 + 每站讲解重点 + 表演时间提醒 + 预计停留 + 推荐理由 + 置信度/评分拆解”。
...
5. Route Memory Agent：记住游客偏好、上一条路线、删减原因和最近对话，支持“太累了/缩短一点/从这里出发”等多轮重规划。
...
- Route Planner Agent：受约束路线规划器。内部由主题识别、候选景点过滤、时间/体力/人群/拥挤度评分、站点排序裁剪、置信度拆解组成；LLM 只用于自然语言解释和讲解润色，不直接决定路径。
- Route Memory Agent：轻量会话记忆层，保存游客偏好、当前路线、已删除站点、拥挤点和最近重规划操作；LLM/规则只解析“操作”，最终路线仍由 Route Planner Agent 生成。
...
- 自然语言路线推荐：从游客口语输入解析出时长、同行、体力、兴趣和避拥挤偏好。
- 多轮路线重规划：基于 Route Memory Agent 支持缩短路线、少走路、避开拥挤、多拍照、多历史和从当前位置出发。
