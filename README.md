...
中国软件杯 A5「景区导览服务 AI 数字人」项目。当前版本已完成游客端 QA、mock 识景、mock 路线推荐、自然语言路线推荐、Route Memory Agent、模拟拥挤度分流、景区导览图拓扑路线规划、运营事件控制台、知识缺口闭环、评测看板、Kiosk 二维码路线带走、本地交互日志/反馈洞察和数字人语音演示闭环；默认无 API Key 可运行。
...
- 景区导览图拓扑路线规划：基于用户提供的线路导览图、手绘观光车图和 Bing 地图，抽象出中轴线、宝藏东线、愿心西线、出口线和拈花湾环线；22 个景点全部映射到拓扑节点，用于顺路指数、步行估算和回头路风险说明。该能力不是 GPS 导航。
...
1. Task UI-06：把 `route_topology` 展示到游客端路线 tab 和 `/route/:id/share` 分享页，让评委直接看到顺路指数、步行估算、游线名称、观光车建议和“非 GPS 导航”边界。
2. Task DOC/DEMO：把景区拓扑路线规划加入 7 分钟演示脚本和技术叙事。
3. Task 07.6：预研 OpenAvatarChat + LiteAvatar sidecar。该 sidecar 只负责数字人表现层，不接管 RAG、路线、识景等业务大脑；失败时必须回退当前 mock 数字人。
4. 后续再考虑真实 TTS provider、真实语音识别或真实 VLM，但必须保持 mock 模式无 API Key 可运行。
